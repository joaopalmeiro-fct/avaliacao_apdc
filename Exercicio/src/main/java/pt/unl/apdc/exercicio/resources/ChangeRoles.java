...
import pt.unl.apdc.exercicio.util.ChangeRolesData;
...
public class ChangeRoles {
	...
	public ChangeRoles () { };	//Nothing to be done

	@POST
	@Path("/roles")
	@Consumes (MediaType.APPLICATION_JSON)
	public Response changeRoles (ChangeRolesData data) {
		LOG.fine("Attempt to change roles");
		
		Key userKey = datastore.newKeyFactory().setKind("User").newKey(data.username);
		Key otherUserKey = datastore.newKeyFactory().setKind("User").newKey(data.otherUser);
		...
		try {
			...
			otherUser = Entity.newBuilder(datastore.get(otherUserKey))
					...
					.set("role", data.role)
					.set("state", otherUser.getString("state"))
